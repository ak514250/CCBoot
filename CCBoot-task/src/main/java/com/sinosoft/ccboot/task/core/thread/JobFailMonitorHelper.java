package com.sinosoft.ccboot.task.core.thread;

import com.sinosoft.ccboot.task.core.model.XxlJobGroup;
import com.sinosoft.ccboot.task.core.model.XxlJobInfo;
import com.sinosoft.ccboot.task.core.model.XxlJobLog;
import com.sinosoft.ccboot.task.core.schedule.XxlJobDynamicScheduler;
import com.sinosoft.ccboot.task.core.util.I18nUtil;
import com.sinosoft.ccboot.task.core.util.MailUtil;
...
/**
 * templates.job monitor instance
			...
			public void run() {
				...
				while (!toStop) {
					try {
						...
						if (CollectionUtils.isNotEmpty(jobLogIdList)) {
							for (Integer jobLogId : jobLogIdList) {
								...
								if (IJobHandler.SUCCESS.getCode() == log.getTriggerCode() && log.getHandleCode() == 0) {
									...
									logger.info(">>>>>>>>>>> templates.job monitor, templates.job running, JobLogId:{}", jobLogId);
								} else if (IJobHandler.SUCCESS.getCode() == log.getHandleCode()) {
									// templates.job success, pass
									logger.info(">>>>>>>>>>> templates.job monitor, templates.job success, JobLogId:{}", jobLogId);
									...
									// templates.job fail,
									failAlarm(log);
									logger.info(">>>>>>>>>>> templates.job monitor, templates.job fail, JobLogId:{}", jobLogId);
								} else {
									JobFailMonitorHelper.monitor(jobLogId);
									logger.info(">>>>>>>>>>> templates.job monitor, templates.job status unknown, JobLogId:{}", jobLogId);
								}
							}
						}
						...
						logger.error("templates.job monitor error:{}", e);
					}
				}
				...
				if (jobLogIdList!=null && jobLogIdList.size()>0) {
					for (Integer jobLogId: jobLogIdList) {
						...
						if (ReturnT.FAIL_CODE == log.getTriggerCode()|| ReturnT.FAIL_CODE==log.getHandleCode()) {
							// templates.job fail,
							failAlarm(log);
							logger.info(">>>>>>>>>>> templates.job monitor last, templates.job fail, JobLogId:{}", jobLogId);
						}
					}
				}
